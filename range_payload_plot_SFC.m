...
for i=1:num
    ...
    for j=1:num
        j;
        for k=1:length(sfc_cruise)
            wffuel(i,j,k)=wftransport(range(i,j),sfc_cruise(k),Vcruise,LD);
...
contour(range,w_payload,w_mtow(:,:,4),[0 1.3e6],'ShowText','off','LineColor','g')
%%
ylabel('Payload (lbs)')
xlabel('Range (nm)')
zlabel('Gross Weight (lb)')
legend('SFC 0.3,','SFC 0.4','SFC 0.5','SFC 0.6')