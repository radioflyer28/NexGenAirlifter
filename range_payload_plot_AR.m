...
for i=1:num
    ...
    for j=1:num
        j;
        for k=1:length(LD)
            wffuel(i,j,k)=wftransport(range(i,j),sfc_cruise,Vcruise,LD(k));
...
hold on
%% EDIT
contour(range,w_payload,w_mtow(:,:,1),[0 1.3e6],'ShowText','off','LineColor','k')
...
contour(range,w_payload,w_mtow(:,:,4),[0 1.3e6],'ShowText','off','LineColor','r')
contour(range,w_payload,w_mtow(:,:,5),[0 1.3e6],'ShowText','off','LineColor','b')
contour(range,w_payload,w_mtow(:,:,6),[0 1.3e6],'ShowText','off','LineColor','c')
%%
ylabel('Payload (lbs)')
xlabel('Range (nm)')
zlabel('Gross Weight (lb)')
legend('AR 5','AR 6','AR 7','AR 8','AR 9','AR 10')